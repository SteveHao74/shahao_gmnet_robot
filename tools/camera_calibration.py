...
@LastEditTime : 2020-01-03 18:53:54
@LastEditors  : Lai
'''
import os
import sys
...
class Calibrater(object):
    ...
    def video_loop(self):
        if self.auto.get():
            ...
            if self.auto_counter >= 15:
                self.auto_counter = 0
                self.save = True
        rgb = self.camera.read_color(5)
        # _, color_image, depth_image = self.camera.read()
        bgr = rgb[..., ::-1].copy()
        bgr = cv2.cvtColor(rgb.copy(), cv2.COLOR_RGB2BGR)
        bgr_ = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY).copy()
        ret, centers = cv2.findCirclesGrid(bgr, self.obj_size, cv2.CALIB_CB_SYMMETRIC_GRID)
        if self.save:
            if ret:
        ...
        if ret:
            # 如果找到标定板则画上找到的点
            cv2.drawChessboardCorners(bgr, self.obj_size, centers, ret)
        plt.clf()
        plt.axis('off')
    ...
    def calibration(images, obj_size=(7, 7), obj_dis=30):
        ...
        for img in images:
            ...
            ret, centers = cv2.findCirclesGrid(img, obj_size, cv2.CALIB_CB_SYMMETRIC_GRID)
            if ret:
...
if __name__ == "__main__":
    out_path = os.path.join(root_path, 'cameras/images/realsense')
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    # with RealsenseSensor(align_to='depth', ) as camera:
    with RealsenseSensor(align_to='color', use='color') as camera:
        UI = Calibrater(out_path, camera, OBJ_SIZE, OBJ_DIS)
        # images = np.hstack((color_image, depth_colormap))
        # Show images
        # while True:
        #     rgb = camera.read_color(2)
        #     cv2.namedWindow('RealSense', cv2.WINDOW_AUTOSIZE)
        #     cv2.imshow('RealSense', rgb[..., ::-1])
        #     cv2.waitKey(30)
