# ur5逆运动学解算器,来自https://github.com/fjonath1/python_UR5_ikSolver,这个比较准
# import PyKDL
    ...
    def getTheta23(self):
        ...
        for i in range(2):
            ...
            for j in range(2):
                ...
                self.theta2[i, j, 0] = -atan2(P13[1], -P13[0]) + \
                    asin(self.a[2]*sin(self.theta3[i, j, 0])/np.linalg.norm(P13))
                self.theta3[i, j, 1] = -self.theta3[i, j, 0]
                self.theta2[i, j, 1] = -atan2(P13[1], -P13[0]) + \
                    asin(self.a[2]*sin(self.theta3[i, j, 1])/np.linalg.norm(P13))
        if self.debug:
