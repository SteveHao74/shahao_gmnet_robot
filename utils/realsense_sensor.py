...
class RealsenseSensor(CameraSensor):
    ...
    def __init__(self, rt=None, align_to=None, use='depth', insces_path=None):
        self.size = np.array([self.IM_WIDTH, self.IM_HEIGHT])
    ...
    def print_camera_info(self):
        ...
        print('mtx:\n', self.mtx)
        print('dist:\n', self.dist)
        print('image size:', (self.IM_HEIGHT, self.IM_WIDTH))

    def start(self):
        """ 每次启动相机的内外参都会有微小的变化,所以每次启动时候重新读取内外参 """
        self.pipeline = rs.pipeline()
        config = rs.config()
        config.enable_stream(rs.stream.depth, self.IM_WIDTH,
                             self.IM_HEIGHT, rs.format.z16, self.FPS)
        config.enable_stream(rs.stream.color, self.IM_WIDTH,
                             self.IM_HEIGHT, rs.format.rgb8, self.FPS)
