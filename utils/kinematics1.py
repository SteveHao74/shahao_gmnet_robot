""" UR5逆运动学求解器,来自https://github.com/littleblakew/UR_kinematics_solver """
...
def ur2np(ur_pose):
    dcm = Rotation.from_rotvec(ur_pose[-3:]).as_dcm()
    m = np.eye(4)
    m[:3, :3] = dcm
    m[:3, 3] = ur_pose[:3]
    return m
...
def inv_kin(p, q_d, i_unit='r', o_unit='r'):
    ...
    for i in range(2):
        for j in range(2):
            ...
            c1 = cos(theta1[i])
            s5 = sin(theta5[j])
            theta6.append(atan2((-T_60[1, 0] * s1 + T_60[1, 1] * c1) /
                                s5, (T_60[0, 0] * s1 - T_60[0, 1] * c1) / s5))
    for i in range(2):
        theta[i, 5] = theta6[0]
    ...
    for i in range(8):
        ...
        if i in [0, 2, 4, 6]:
            theta[i, 2] = -cmath.acos((np.linalg.norm(P_13) ** 2 - a2 **
                                       2 - a3 ** 2) / (2 * a2 * a3)).real
            theta[i+1, 2] = -theta[i, 2]
        # theta2
        theta[i, 1] = -atan2(P_13[1], -P_13[0]) + \
            asin(a3 * sin(theta[i, 2]) / np.linalg.norm(P_13))
        # theta4
